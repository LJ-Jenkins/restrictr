...
restrict <- function(
  ...
  for (i in seq_along(rargs)) {
    ...
    mask <- check_coerce_type(args, mask, arg, .env, .class, .error_call)

    mask <- check_coerce_size(args, mask, arg, .env, .class, .error_call)

    check_validates(args, mask, arg, .env, .class, .error_call)
