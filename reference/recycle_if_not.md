...
size specified in the expression. Expressions are evaluated and objects
are assigned into the environment specified by the `.env` argument. The
...
- .env:
  ...
  and the assignment of the recycled objects. Cannot be the global
  environment.
...
#>   Caused by error in `recycle_if_not()`:
#> ℹ In argument: `x = y`.
#> ! Size argument is length `3`, needs to be positive scalar integerish.
# Error:
# Caused by error in `recycle_if_not()`:
# ℹ In argument: `x = y`.
